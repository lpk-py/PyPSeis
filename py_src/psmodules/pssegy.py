...
#     Modified from Thomas Mejer Hansen, 2005
#
...
SH_def["SegyFormatRevisionNumber"]={"pos": 3500,"type":"uint16","def":256}
...
def main():
	filename = '/Users/Uqer/Dropbox/SEGY/s41554.sgy'
	
	# verbose =1;
	# Data, SH, STH=readSegy(filename)
	# imageSegy(Data)
...
def wiggleVAll(Data,SH,skipt=1,scale=0.5, ucolor='blue',lwidth=.1):
	"""
	wiggle(Data,SH)
	"""
...
def wiggleVComp(Data,SH,component=1,scale=0.5, ucolor='blue', lwidth=.1):
	"""
	wiggle(Data,SH)
	"""
	...
	if (component !=3):
		...
		#		trace[SH['ns']+1]=0
			...
			pylab.plot(1+i/3 + trace / maxval * scale, t, color=ucolor, linewidth=lwidth)
			for a in range(len(trace)):
				if (trace[a] < 0):
					trace[a] = 0;
				# pylab.fill(i+Data[:,i]/maxval, t, 'k', linewidth=0)
...
def getDefaultSegyHeader(ntraces=1,ns=100):
	"""
	...
	return SH
def getDefaultSegyTraceHeaders(ntraces=1,ns=100,dt=1000):
	"""
...
def getSegyTrace(filename):
	...
	# GET TRACE
	index=3200+(6)*(240+SH['ns']*bps)+240
	SegyTraceData = getValue(data,index,'f',endian,SH['ns'])
	return SegyTraceData
